...
class NoteEditorViewController: UIViewController, UITextViewDelegate {
    ...
    private func textStyleMenu() -> UIMenu {
            // Helper to generate a preview image for a given font and text
        func imageForTextStyle(_ text: String, font: UIFont, size: CGSize = CGSize(width: 96, height: 32)) -> UIImage? {
            let renderer = UIGraphicsImageRenderer(size: size)
            let image = renderer.image { ctx in
                    // Fill background transparent
                UIColor.clear.setFill()
                ctx.fill(CGRect(origin: .zero, size: size))
                    // Draw text centered vertically and left aligned
                ...
                let textRect = CGRect(x: 8, y: (size.height - font.lineHeight)/2, width: size.width - 16, height: font.lineHeight)
                (text as NSString).draw(in: textRect, withAttributes: attributes)
            }
            return image
        }
        
        let styles: [(NoteTextStyle, String, UIFont.TextStyle)] = [
        ...
        let actions: [UIAction] = styles.map { style, label, fontStyle in
            let font = UIFont.preferredFont(forTextStyle: fontStyle)
            let image = imageForTextStyle(label, font: font)
